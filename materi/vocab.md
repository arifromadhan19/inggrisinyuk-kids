...
Status: **SEMUA 6 LEVEL DIIMPLEMENTASIKAN** — Little Stars 12 topik, Starter 10 topik, Explorer 10 topik, Adventurer 13 topik, Achiever 10 topik (semua × 10 kata), Trailblazer 2 topik × 10 kata (SENGAJA cuma segini — PRD §9 "low-effort, 1-2 modul preview", bukan gap). Vocabulary skill kini py materi nyata di seluruh tangga level — sesi berikutnya (kalau ada) bergeser ke skill lain (Listening/Speaking/Grammar/Reading), lihat §6.
...
| Trailblazer | 2 (`perjalanan-wisata`, `bahasa-komunikasi`, §5F) | 10 | 20 | ⚠️ SENGAJA di bawah 10 — PRD §9 mengunci "low-effort, 1-2 modul", bukan gap |
...
1. **Id topik Vocabulary global, bukan per-level** (detail §2.2) — cek daftar id yang SUDAH dipakai di §5A (Little Stars), §5B (Starter), §5C (Explorer, termasuk 3 topik lama `keluarga`/`angka`/`warna`), §5D (Adventurer, termasuk 10 topik lama `pekerjaan`…`perasaan`), §5E (Achiever), §5F (Trailblazer) sebelum kasih id topik baru di level manapun — 57 id total sekarang, SEMUA 6 level sudah terisi.
...
**Lokasi kode**: `app/src/content.ts` — `VOCAB_TOPICS_TRAILBLAZER` (const array baru, CUMA 2 topik — sengaja, §3F.1), didaftarkan di `VOCAB_TOPICS_BY_LEVEL['trailblazer']`. `LEVELS` entri `trailblazer`: `hasContent: false → true`. `CONTENT_AVAILABLE` **tidak perlu diubah** — Trailblazer, spt Little Stars & Achiever, di luar `PlacementLevelKey` sama sekali (§4 poin 3); PRD §6 malah eksplisit bilang Trailblazer diakses berbasis usia, bukan hasil placement test sama sekali.

2 topik, tiap topik 10 kata — dipetakan dari 2 tema Cambridge B1 Preliminary (PET) yang genuinely paling segar (§3F.2):
...
- **`bahasa-komunikasi` sengaja jadi topik ke-2 (bukan cuma 1 topik)** meski scope "low-effort" — krn nilai meta-tematiknya tinggi (app pembelajaran bahasa mengajarkan KATA UNTUK bicara tentang bahasa itu sendiri: Translate/Fluent/Accent/Bilingual/Native Speaker) — dinilai layak jadi 1 dari 2 modul preview, bukan sekadar tema acak.
...
- **TIDAK ada topik ke-3, ke-4, dst** — bukan kehabisan ide/waktu, tapi keputusan sadar mengikuti PRD §9. Kalau user nanti minta Trailblazer diperluas (revisi scope), riset §3F.2 sudah menyisakan 20 tema PET lain siap dipetakan — TAPI itu butuh keputusan scope baru dari user dulu (bukan diasumsikan otomatis boleh oleh sesi berikutnya).
...
| **Trailblazer cuma 2 topik Vocabulary** (bukan ≥10) | KEPUTUSAN SCOPE, bukan belum sempat (§3F.1, PRD §9 "low-effort, 1-2 modul") | Sesuai desain — jangan "digenapkan" tanpa arahan baru user |
...
Dijalankan tiap sesi lewat skrip sementara (esbuild+node, tidak disimpan ke repo) yang meng-import `VOCAB_TOPICS_BY_LEVEL` langsung dari `content.ts`:

- **0 id topik Vocab bentrok** lintas SEMUA 6 level (Little Stars 12 + Starter 10 + Explorer 10 + Adventurer 13 + Achiever 10 + Trailblazer 2 = **57 id topik, semuanya unik**).
- **Semua 57 topik ≥10 kata** (syarat CLAUDE.md per-topik) — Trailblazer sengaja cuma 2 TOPIK (bukan ≥10 topik/skill, §3F.1/§5F), tapi tiap topiknya sendiri tetap penuh 10 kata, tidak dikurangi.
- **Semua ~570 kata** lolos cek `example.en` memuat `en` persis sebagai whole word (regex `\b<kata>\b`, case-insensitive) — syarat teknis `blankSentence()`. Satu bug ditemukan & diperbaiki di sesi Starter: item "Twin" (`orang-di-sekitarku`) awalnya punya contoh berbentuk jamak "They are twins." yang GAGAL match — diperbaiki jadi "This is my twin." (§2.2). Sesi Explorer, Adventurer, Achiever & Trailblazer lolos verifikasi tanpa temuan bug baru (pelajaran dari bug Starter diterapkan preventif di semua kalimat kata kerja sejak itu — bentuk dasar, bukan orang-ketiga-tunggal).
- `npm run typecheck` (`tsc --noEmit`, `app/` DAN `portal/` — dua project TypeScript terpisah) dan `npm run build` (esbuild) lulus tanpa error sesudah tiap sesi perubahan `content.ts`/`placement-scoring.ts`.
...
**Inisiatif "materi Vocabulary bertahap per level" SELESAI di sesi ini** — 6 dari 6 level punya materi Vocabulary nyata (57 topik, ~570 kata), semua terverifikasi (id unik, whole-word match, typecheck, build, QA browser). Gap yang tersisa (§6) semuanya di skill LAIN (Listening/Speaking/Grammar/Reading) — kalau user minta lanjut, itu scope baru di luar dokumen ini.
