...
1. `ada-apa-di-sini` ("Ada Apa di Sini? (There Is or There Are?)") — "there is/there are" utk keberadaan, struktur resmi Starters (dikonfirmasi jg Kurikulum Merdeka Fase A kelas 2 SD). `contrastVisual:'quantity'` **REUSE PERSIS** (singular/plural sama dgn `satu-banyak` Little Stars, cuma frame kalimat beda), semula dari Vocab Starter `serangga` (Insects) — **domain katanya kemudian DIGANTI TOTAL ke `perkakas` (Tools & Equipment)**, laporan user soal emoji serangga yg nyaris semua "tubuh utuh" tanpa varian "kepala saja" di Unicode (`materi/vocab.md` §3B.4); id/judul topik grammar ini sendiri TIDAK berubah krn fokusnya struktur "there is/are", bukan domain katanya. Lokasi "here" DIBUAT KONSTAN di semua 10 item (bukan divariasikan per item) — kalau lokasinya ikut beda-beda per item, itu jadi sinyal kedua yang bocor jawaban (pelajaran yg sama §8/§13).
